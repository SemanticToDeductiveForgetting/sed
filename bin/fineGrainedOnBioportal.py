...
java = "/usr/bin/java"
jar = "./restclient-dep-1.0.jar"
fameJar = "./fame-dep-1.0.jar"
letheJar = "./lethe-dep-1.0.jar"
signature = "100"
experiment = ""
ontology = ""
...
plannerCmd = '{java} -cp {jar} {prog} -ontology {ontology} -signature {signature} -outDir {outDir} -quantifiedSignature'.format(java=java,
                                                                                                             jar=jar,
                                                                                                             prog="pc1.pc2.pc3.ExperimentPlannerWithSignaturePortion",
                                                                                                             ontology=ontology,
                                                                                                             outDir=outDir,
                                                                                                             signature=signature)
...
executorCmd = "{java} -cp {jar} {prog} -outDir {outDir}/newRun -sig {sig} -ont {ont} -reduceToDeductive -verbose".format(
    java=java,
    jar=jar,
    prog="pc1.pc2.pc3.SemanticForgetting",
    outDir=outDir,
    sig=sigFile,
    ont=ontologyClausal)
