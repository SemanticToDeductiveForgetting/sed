...
outDir = "c:/ccviews/forget/experiment/NoBG"
sigSize = "10"
experimentName="NA"
...
for opt, arg in opts:
    if opt in ("-n", "--exName"):
        ...
        print('nobgexp.py -n <experimentName> -f <signatureSize>')
        sys.exit()
...
plannerCmd = '{java} -cp {jar} {prog} -repo {repo} -sigSize {sigSize} -outDir {outDir}/data -verbose'.format(java=java,
                                                                                                             jar=jar,
...
###################################################Lethe########################################################
inOnt = "{repo}/{experiment}".format(repo=repository, experiment=experimentName)
